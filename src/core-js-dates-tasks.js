...
function getNextFriday(date) {
  const friday = date;
  const add = [5, 4, 3, 2, 1, 7, 6];
  const dayOfWeek = date.getDay();
  const dayOfMonth = date.getDate();
  friday.setDate(dayOfMonth + add[dayOfWeek]);
  return friday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const date1 = Date.parse(dateStart);
  const date2 = Date.parse(dateEnd);
...
function formatDate(date) {
  ...
  return `${month + 1}/${day}/${year}, ${hours}:${minutes}:${seconds} ${a}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 0; i < daysInMonth; i += 1) {
    const dateTemp = new Date(year, month - 1, i + 1);
...
function getWeekNumberByDate(date) {
  const firstJanDate = new Date(date);
  firstJanDate.setDate(1);
  firstJanDate.setMonth(0);
  const firstJanDay = firstJanDate.getDay();
  let diff = Math.ceil(Math.floor((date - firstJanDate) / 86400000) / 7);
  if (firstJanDay === 3) {
    diff += 1;
  }
  return diff;
...
function getNextFridayThe13th(date) {
  ...
  while (!(day === 5 && monthDay === 13)) {
    tempDate = new Date(Date.parse(tempDate) + 86400000);
    day = tempDate.getDay();
    monthDay = tempDate.getDate();
  }
  return tempDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (Date.parse(tempDate) <= Date.parse(endDate)) {
    let i = countWorkDays;
    while (Date.parse(tempDate) <= Date.parse(endDate) && i > 0) {
      i -= 1;
    ...
    tempDate = new Date(Date.parse(tempDate) + 86400000 * countOffDays);
  }
  return schedule;
}
